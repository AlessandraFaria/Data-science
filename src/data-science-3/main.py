...
def q4():
    fifa_ = fifa.dropna()
    X_rfe = fifa_.drop(columns=['Overall'])
    y_rfe = fifa_['Overall']
    estimator = LinearRegression()
    rfe = RFE(estimator,verbose=1, n_features_to_select=5)
    fifa_rfe = rfe.fit_transform(X_rfe, y_rfe.ravel())
    valores = pd.DataFrame(data=[X_rfe.columns, rfe.support_]).transpose()
    return list(valores[valores[1] == True][0])
